    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i < 11; i++) {
            ...
            for (int x = 0; x < items.size(); x++) {
                ...
                item.put("bindTime",items.getJSONObject(x).getJSONObject("entity").getLongValue("bindTime"));
                item.put("userId",items.getJSONObject(x).getJSONObject("entity").getInteger("userId"));
                item.put("wechatName",items.getJSONObject(x).getJSONObject("entity").getString("wechatName"));
                item.put("days",items.getJSONObject(x).getJSONObject("entity").getInteger("days"));
                item.put("userId",items.getJSONObject(x).getJSONObject("entity").getInteger("userId"));
                result.add(item);*/
